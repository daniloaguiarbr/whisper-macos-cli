...
containers. The video file audio track is extracted to a temporary
...
- env_clear: no host environment variables are inherited except
  a minimal allowlist (`PATH`, `HOME`, `TMPDIR`, `LANG`, `LC_ALL`).
  Secrets like `*_TOKEN` cannot leak into ffmpeg logs.
- setsid on Unix / CREATE_NEW_PROCESS_GROUP on Windows: the child
  runs in its own process group. Ctrl+C delivered to the parent
  does not silently propagate to ffmpeg.
- Kill-on-drop: the child handle is wrapped in a `SafeChild` guard.
  If the parent panics, the child is killed (SIGKILL on Unix,
...
- Temp cleanup: the temp WAV is removed via a `Drop` guard even
  if decode panics.

## Limits

- Maximum duration: 24 hours (inherited from the audio pipeline).
- Maximum file size: limited by the temp directory; ~3 GB practical
  ceiling for 1h of typical video.
...
- [skill/whisper-macos-cli-en/SKILL.md](../skill/whisper-macos-cli-en/SKILL.md) — JSON contract reference
